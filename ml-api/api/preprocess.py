...
def get_grayscale(filenames: list):
...
def shrink_image(
    ...
    """Reduce image size to 8x8 pixels and convert to 16-level grayscale"""
    ...
    if width > hight:
        ...
        center = (h_max + h_min) // 2
        left = (center - hight // 2) - 1 - offset
        upper = v_min - offset
        right = (center + hight // 2) + offset
        lower = v_max + offset

    img_croped = img.crop((left, upper, right, lower)).resize(
        (crop_size, crop_size)
    )
    ...
    img_data16 = (
        (img_data256 - min_bright) / (max_bright - min_bright) * max_size
    )
    return img_data16


def get_shrinked_img(filenames: list[str]):
    """Create a numerical data list for images"""
